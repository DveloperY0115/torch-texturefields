...
parser.add_argument(
    "--device_id", type=int, default=3, help="CUDA device ID if multiple devices available"
)
parser.add_argument(
...
parser.add_argument("--test_dataset_size", type=int, default=1000, help="Cardinality of test set")
...
# ----- Checkpoint -----
parser.add_argument("--ckpt", type=str, required=True, help="[Inference] Path to checkpoint file used for inference")
parser.add_argument("--inf_out", type=str, required=True, help="[Inference] Directory where inference results will be stored")
args = parser.parse_args()

if __name__ == "__main__":
    trainer = TextureFieldsConditionalTrainer(opts=args, checkpoint=args.ckpt)
    trainer.inference(args.inf_out)
